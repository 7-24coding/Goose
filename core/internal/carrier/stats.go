...
// GetStats returns the current total transfer statistics and API usage.
func (c *Client) GetStats() (bytesIn, bytesOut, dailyCount, scriptCount uint64) {
	...
	for i := range c.endpoints {
		ep := &c.endpoints[i]
		c.touchDailyWindow(ep, now)
		dailyCount += ep.dailyCount
		scriptCount += ep.scriptCount
	}
	return
}

// EndpointStat represents stats for a single script URL
...
// GetEndpointStats returns usage statistics per endpoint
func (c *Client) GetEndpointStats() []EndpointStat {
	c.endpointMu.Lock()
	defer c.endpointMu.Unlock()
	
	now := time.Now()
	stats := make([]EndpointStat, 0, len(c.endpoints))
	...
	for i := range c.endpoints {
		...
		stats = append(stats, EndpointStat{
			...
			DailyCount:  ep.dailyCount,
			ScriptCount: ep.scriptCount,
			IsBlacklisted: ep.blacklistedTill.After(now),
		})
	}
...
// MeasurePing measures the round-trip latency to the first configured script URL.
func (c *Client) MeasurePing(ctx context.Context) (time.Duration, error) {
	c.mu.Lock()
	hasSessions := len(c.sessions) > 0
	c.mu.Unlock()
	if !hasSessions {
		return 0, ErrNoActiveSessions
	}

	c.endpointMu.Lock()
	if len(c.endpoints) == 0 {
		c.endpointMu.Unlock()
		return 0, fmt.Errorf("no endpoints configured")
	}
...
// MeasurePingManual measures the round-trip latency to the first configured script URL, bypassing session presence checks.
func (c *Client) MeasurePingManual(ctx context.Context) (time.Duration, error) {
	c.endpointMu.Lock()
	if len(c.endpoints) == 0 {
		c.endpointMu.Unlock()
		return 0, fmt.Errorf("no endpoints configured")
