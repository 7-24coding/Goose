...
type Config struct {
	...
	// DisableIdleSleep when true forces workers to poll even with 0 active sessions.
	// Used by unit tests to verify idle-polling behavior.
...
func New(cfg Config) (*Client, error) {
	...
	return &Client{
		...
		disableIdleSleep:   cfg.DisableIdleSleep,
	}, nil
}
...
func (c *Client) runWorker(ctx context.Context) {
	...
	for {
		...
		c.mu.Lock()
		hasSessions := len(c.sessions) > 0 || c.disableIdleSleep
		c.mu.Unlock()
...
func idleBackoff(n int) time.Duration {
	...
	default:
		// After ~2 minutes of zero traffic, completely halt polling to save quota.
		// Any local traffic will instantly wake workers via wake.Broadcast().
		return 24 * time.Hour
	}
}
